...
Session.prototype.begin = function(sessionPolicy) {
  ...
  if (this.policy.restart) {
    this._timeouts = u.generateTimeouts(this.policy.restart);
  }
...
Session.prototype._processEndFrame = function(frame) {
  ...
  if (this.sm.getMachineState() !== 'UNMAPPED') {
    ...
    // Restart the session if the end was initiated by the broker
    // and the policy indicates we should
    if (this._shouldRestart()) {
      var self = this;
      this.once(Session.Unmapped, function () {
        self._attemptRestart();
      });
    }
  }
...
Session.prototype._shouldRestart = function() {
  return this._timeouts && (this._timeouts.length || this.policy.restart.forever);
};

Session.prototype._attemptRestart = function() {
  ...
  this._restartTimer = setTimeout(function() {
    if (self._shouldRestart() && self.connection.connected) {
      debug('attempting to restart session');
      self.begin(self.policy);
    } else {
      process.nextTick(function() { self._attemptRestart(); });
    }
